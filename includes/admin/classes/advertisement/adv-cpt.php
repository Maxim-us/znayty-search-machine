...
class MXZSMADVCPTclass
{

	static public function createAdvCPT()
	{
		
		add_action( 'init', array( 'MXZSMADVCPTclass', 'mxzsm_adv_custom_init' ) );
		...
		static public function mxzsm_adv_custom_init()
		{
			...
			$_menu_title = 'Мені потрібно';

			$count_v_posts = self::mxzsm_count_of_verification_posts();

			if( $count_v_posts !== false ) {

				$_menu_title =  'Мені потрібно (' . $count_v_posts . ')';

			}

			register_post_type( 'mxzsm_adv_need', array(

				'labels'             => array(
					'name'               => 'Мені потрібно',
					'singular_name'      => 'Мені потрібно',
					'add_new'            => 'Додати нове',
			...
			// new post status
			register_post_status( 'verification_need', array(
	            ...
	            'post_type'                 => array( 'mxzsm_adv_need' ),
	        ...
	        // category
			register_taxonomy( 'mxzsm_adv_category', [ 'mxzsm_adv_need' ], [ 
		...
		public static function mxzsm_count_of_verification_posts()
		{
			...
			$posts_results = $wpdb->get_results( 
				"SELECT ID FROM $posts_table
					WHERE post_status = 'verification_need'"
			);
